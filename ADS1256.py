...
class ADS1256:
    def __init__(self):
        # Open gpiod chip and get line handles
        self.chip = gpiod.Chip('gpiochip4')
        self.rst_line = config.RST_LINE
    ...
    def ADS1256_reset(self):
        config.digital_write(self.rst_line, 1)
        config.delay_ms(200)
        config.digital_write(self.rst_line, 0)
        config.delay_ms(200)
        config.digital_write(self.rst_line, 1)
    ...
    def ADS1256_WriteCmd(self, reg):
        ...
        config.digital_write(self.cs_line, 1) #cs 1
    ...
    def ADS1256_WaitDRDY(self):
        for i in range(0,400000,1):
            if(config.digital_read(self.drdy_line) == 0):
